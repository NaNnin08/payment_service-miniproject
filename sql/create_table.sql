...
select CONCAT('PAAC',to_char(now(),'YYYY'),to_char(now(),'MM'),to_char(now(),'DD'),'-',nextval('seq_paac_number'))
$$ language sql

create or replace function pyt_trx () returns varchar as $$
select CONCAT('PAY',to_char(now(),'YYYY'),to_char(now(),'MM'),to_char(now(),'DD'),'-',nextval('seq_pyt_trx'))
$$ language sql
